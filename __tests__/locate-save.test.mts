...
function harness(initialCodes = ['A1'], refresh: () => Promise<void> = async () => {}) {
  ...
  const deps: Record<string,any> = {
    ...
    '@/store':{useStore:()=>({boot:{assets:{A1:{c:'Customer',or:1}}},refresh,outbox:{scans:[]}})},
    '@/ui':{useBottomInset:()=>0},
    '@/live':{useLiveData:()=>{}},
...
test('a saved shelf clears even when the catalogue refresh never answers',async()=>{
  // 22 Sep 2026: bottles "stuck at mark as full even after submitting" — the
  // save had landed, but the screen waited on a bootstrap download first.
  const h=harness(['A1','B2'],()=>new Promise<void>(()=>{}));
  const save=h.screen.save();
  h.pending[0].resolve(h.result);
  await save;
  assert.deepEqual(Array.from(h.state[4]),[]);
  assert.equal(h.alerts.length,1);
});
test('a bottle scanned while the save was in flight stays staged',async()=>{
  const h=harness(['A1']);
  const save=h.screen.save();
  h.state[4]=[...h.state[4],'C3'];
  h.pending[0].resolve(h.result);
  await save;
  assert.deepEqual(Array.from(h.state[4]),['C3']);
});
